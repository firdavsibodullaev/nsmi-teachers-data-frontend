    ...
    <div class="space-align-container">
      <div class="links">
        <a-button type="primary" v-if="this.$store.getters['user/user'].post.name === 'Админ'"
          ...
          <router-link :to="{name: 'article_pending_confirmation'}">Не подтвержденные</router-link>
        </a-button>
        <a-button type="primary" style="margin-left: 5px;">
          <router-link :to="{name: 'article_create'}">Добавить</router-link>
        </a-button>
      </div>
    </div>
    <a-table
...
export default {
  ...
  methods: {
    ...
    fetch(params = {}) {
      this.$api.getArticles({...params}, ({data}) => {
        this.data = data.data;
        this.pagination = {
